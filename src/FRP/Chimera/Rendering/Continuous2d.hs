...
renderFrameCont2d ar er winSize@(wx, wy) t ss e = 
    GLO.Pictures (envPic : agentPics)
  where
...
defaultAgentRendererCont2d :: Float 
                           -> AgentColorerCont2d s 
                           -> AgentCoordCont2d s 
                           -> AgentRendererCont2d s
defaultAgentRendererCont2d size acf apf (sx, sy) (wx, wy) _t (_, s) = 
    GLO.color color $ GLO.translate xPix yPix $ GLO.ThickCircle 0 size
  where
    (x, y) = apf s
    color = acf s
