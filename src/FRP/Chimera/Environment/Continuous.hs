...
wrapCont2d :: Continuous2dDimension 
           -> EnvironmentWrapping 
           -> Continuous2dCoord 
           -> Continuous2dCoord
wrapCont2d (maxX, maxY) ClipToMax (x, y) = 
                (max 0 (min x maxX), max 0 (min y maxY))
...
wrapCont2d l@(_, maxY) WrapVertical (x, y)
  ...
  | otherwise = (x, y)
wrapCont2d l WrapBoth coord = 
                wrapCont2d l WrapHorizontal $ wrapCont2d l WrapVertical coord
...
-- NOTE: use currying to fix the origin in the distance-function 
--       e.g. distanceManhattanCont2d or distanceEuclideanCont2d
objectsInDistance :: Ord o
                  => Double 
                  -> DistanceFunction
                  -> Continuous2d o
                  -> [o]
